...
export default class PixelHeartSpriteSheet extends GameObject {
    ...
    camera = null;
    selectedImage;
    #mouseTargetPosition = null;
    constructor(sheetExport = null) {
        super();
        if (sheetExport)
            this.import(sheetExport);
        else {
    ...
    get currentImage() {
        return this.sheetContent.images.find(image => image.position[0] === this.selectedImage[0] &&
            image.position[1] === this.selectedImage[1]).image;
    }
    bootEvents() {
        window.addEventListener('PushCommand', ({ detail: { command } }) => this.currentImage.pushCommand(command));
    ...
    import(SheetExport) {
        ...
        let first = this.sheetContent.images.reduce((image, res) => {
            ...
            if (image.position[1] > res.position[1])
                return res;
            if (image.position[0] < res.position[0])
                return image;
            else
                return res;
        });
    ...
    #mouseUpdate(dt) {
        ...
        if (mouse.right) {
            if (!this.#mouseTargetPosition)
                this.#mouseTargetPosition = mouse.position.clone();
            let delta = this.#mouseTargetPosition.clone().sub(mouse.position);
